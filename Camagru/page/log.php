<?php 
session_start();
 ?>
<html>
	<head>
		<meta charset='utf-8'/>
		<title>Camagru_42</title>
		<link rel='stylesheet' href='style.css'/> 
	</head>
	<body>
		<h1 class='home'>Bienvenue !</h1>
		<div class="flexband">
 			<a href="gallery.php"><button class="flexbut">Galerie</button></a>
 		</div>
		<div class='home'>
			<p>Tout doux minou. Déja inscrit?<br/>
			<a href='login.php'><input type='submit' value='Connection'></a><br/><br/><br/>
			Inscrit toi, tout va bien ce passer.<br/>
			<a href='register.php'><input type='submit' value='Inscription'></a><br/><br/>
			Tu as oublié tes identifiants?<br/>
			<a href="get_passwd.php"><input type="submit"
			value="mot de passe oublié"></p>
		</div>
	</body>
</html>