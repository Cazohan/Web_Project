...
var url = "mongodb://RandomUser:MatchaDB@localhost:28000/matcha";
var utilities = require('./utility');
var googleMap = require('@google/maps').createClient({
    key:'AIzaSyDAXSneBT2_SNqpKRdQBEHnK4aq0OunZ_E'
});
        ...
        MongoClient.connect(url, function(err, db) {
            ...
            collec.find({}).toArray(function(err, docs) {
                ...
                if (ok == 1) {
                    ...
                    db.close();
                    callback(null, { mess: 'Connexion Réussi !!!' }, {log:log,myId:docs[j]['myId']}, '/');
                } else {
                    req.flash('error', 'Utilisateur inconnue');
                    db.close();
                    callback('Utilisateur inconnue', null, null, 'login');
                }
            });
        });
    } else {
        req.flash('error', 'T\'ho ! Informations manquantes');
        callback('T\'ho ! Informations manquantes', null, null, 'login');
    }
};
